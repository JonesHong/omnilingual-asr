...
class StreamingResult:
    ...
    start_timestamp: float = 0.0  # The start timestamp of this text
    is_stable: bool = True        # Whether this result is stable (final correction) or unstable (preview)
...
class VADSegmenter:
    ...
    def get_speech_segments(self) -> Iterator[Tuple[np.ndarray, float, bool]]:
        ...
        if self.vad_model is None:
            # No VAD, just yield chunks based on max_segment_duration
            # This acts as a fallback "fixed chunk" mode if VAD fails or is disabled
            chunk_size = int((self.max_segment_duration_ms / 1000.0) * self.sample_rate)
            if chunk_size <= 0:
                chunk_size = int(3.0 * self.sample_rate) # Default to 3s
...
class StreamingASRPipeline:
    """
    Unified Streaming ASR Pipeline supporting VAD, Two-Pass Strategy, and multiple model types.
    
    Replaces the legacy stride-based pipeline with a more robust VAD-based approach.
    """
    ...
    def __init__(
        ...
        # New parameter for model sharing
        ...
        self.dtype = dtype
        self.lang = lang
        self.enable_stability_pass = enable_stability_pass
        
        # Initialize Base Pipeline to load model
        if base_pipeline is not None:
            # Use provided pipeline (shared model)
            self.base_pipeline = base_pipeline
            print(f"Using shared model: {model_card}")
        else:
            # Load new model
        ...
        if use_vad:
            ...
            # If VAD is disabled, we use the segmenter in a "fixed chunk" mode
            # We can use max_segment_duration_ms to control the chunk size
            self.segmenter = VADSegmenter(
                max_segment_duration_ms=max_segment_duration_ms
            )
            self.segmenter.vad_model = None  # Explicitly disable VAD model
        
        # Context Buffer for Two-Pass Strategy
        self.context_buffer: List[np.ndarray] = []
        self.context_duration_ms = context_duration_ms
        self.context_start_time = 0.0
        
        print(f"Streaming pipeline initialized. VAD: {use_vad}, Stability pass: {enable_stability_pass}")
    ...
    def transcribe_available(self) -> Iterator[StreamingResult]:
        ...
        for segment, timestamp, is_final in self.segmenter.get_speech_segments():
            ...
            # 1. Fast Path: Transcribe short segment immediately
            text = self._process_segment(segment)
            segment_duration = len(segment) / 16000
            start_timestamp = timestamp - segment_duration
            
            if text:
                # If stability pass is disabled, this result is stable (final)
                is_stable = not self.enable_stability_pass
            ...
            # 2. Slow Path: Accumulate to context buffer (only if enabled)
            if self.enable_stability_pass:
                self.context_buffer.append(segment)
                
                # Check accumulated duration
                current_context_samples = sum(len(s) for s in self.context_buffer)
                current_context_duration_ms = (current_context_samples / 16000) * 1000
                
                if current_context_duration_ms >= self.context_duration_ms:
                    # Trigger stable correction
                    full_audio = np.concatenate(self.context_buffer)
                    stable_text = self._process_segment(full_audio)
                    
                    end_timestamp = self.context_start_time + (len(full_audio) / 16000)
                    ...
                    if stable_text:
                        yield StreamingResult(
                            ...
                            latency=0.0,
                            start_timestamp=self.context_start_time,
                            is_stable=True  # Mark as stable (correction)
                        )
                    
                    # Reset context buffer
                    self.context_buffer = []
                    self.context_start_time = end_timestamp

    def finish(self) -> Iterator[StreamingResult]:
        """Process remaining audio."""
        # First, process any remaining segments from VAD
        ...
        # Then, process any remaining context buffer as stable
        if self.enable_stability_pass and self.context_buffer:
            full_audio = np.concatenate(self.context_buffer)
            stable_text = self._process_segment(full_audio)

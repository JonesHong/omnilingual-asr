...
def test_streaming_llm(audio_path: str, model_card="omniASR_LLM_300M", lang="cmn_Hant"):
    ...
    while cursor < len(audio):
        ...
        for result in pipeline.transcribe_available():
            ...
            print(f"{status} [{result.timestamp:.2f}s] {result.text}")
...
if __name__ == "__main__":
    ...
    if audio_path.exists():
        # Test with LLM model
        test_streaming_llm(str(audio_path), model_card="omniASR_LLM_300M", lang="cmn_Hant")
    else:
